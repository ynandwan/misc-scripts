...
if timing is None:
    timing = [args.global_time]*len(cross_product_params[0]['values'])

assert len(all_params[names.index(timing_key)]) == len(
    timing), 'len of timing should be same as len of timing_key param'

timing_dict = dict(zip(all_params[names.index(timing_key)], timing))
jobs = list(itertools.product(*all_params))

additional_short_names = config['param_groups']['short_names']#  ['agtr','rr','sll','tll','b','fa','ans','ns','e','ne']
additional_names = config['param_groups']['names']# ['aug_trans','rev_reg','sym_loss_lambda','trans_loss_lambda','batch','filter_annotated','aug_num_sentences','num_sentences','epochs','num_evals']
...
for param_name in sorted_names:
    if param_name in config['parse_value_fns']:
        transform_fns[param_name] = getattr(hpc_utils, config['parse_value_fns'][param_name])
    else:
...
for i, setting in enumerate(all_jobs):
    ...
    log_str = '_'.join(['%s-%s' % (hpc_utils.replace_spl(name_to_short[n]),
                                   transform_fns[n](str(name_setting[n]))) for n in sorted_names])
    print(log_str)
    ...
    setting_list = ['--%s %s' % (name, str(value))
                    for name, value in name_setting.items() if name not in do_not_pass_params]
...
for log_str in sorted_job_names:
    ...
    if count % args.num_task_per_process == 0:
        ...
        if jcount % args.num_process_per_job == 0:
            ...
            header = '#PBS -N {}_mn_{}_{}'.format(
                args.job_name, mjcount, log_str[:10])
...
for k,v in buckets.items():
    fname = os.path.join(os.getenv('PWD'), args.jobs_dir, args.prefix+'all_commands_'+str(k)+'.sh')
